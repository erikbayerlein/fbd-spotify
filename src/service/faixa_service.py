...
class FaixaService:
    ...
    def add_to_db(self, id_album, album_enviroment):
        ...
        for i in range(0, num):
            opt_compositor = str(input(f"Voce gostaria de adicionar um novo compositor a faixa {i+1}? (sim/nao): ")).lower()
            ...
            if album_enviroment == "cd" and faixa.num_cd != None:
                sql_query_faixa = f"faixa (id_faixa, id_album, id_tipo_comp, descricao, tempo_de_exec, pos_album, num_cd) VALUES ('{faixa.id}', '{faixa.id_album}', '{faixa.id_composition}', '{faixa.descr}', '{faixa.exec_time}', '{faixa.position_album}', '{faixa.num_cd}')"
            elif album_enviroment == "vinil" and faixa.num_vinil != None:
                sql_query_faixa = f"faixa (id_faixa, id_album, id_tipo_comp, descricao, tempo_de_exec, pos_album, num_vinil) VALUES ('{faixa.id}', '{faixa.id_album}', '{faixa.id_composition}', '{faixa.descr}', '{faixa.exec_time}', '{faixa.position_album}', '{faixa.num_vinil}')"
            else:
                sql_query_faixa = f"faixa (id_faixa, id_album, id_tipo_comp, descricao, tempo_de_exec, pos_album) VALUES ('{faixa.id}', '{faixa.id_album}', '{faixa.id_composition}', '{faixa.descr}', '{faixa.exec_time}', '{faixa.position_album}')"
            DataBaseService().insert(sql_query_faixa)

            sql_query_faixa_compositor = f"faixa_compositor (id_faixa, id_compositor) VALUES ('{faixa.id}', '{compositor_id}')"
            DataBaseService().insert(sql_query_faixa_compositor)

            sql_query_faixa_interp = f"faixa_interprete (id_faixa, id_interprete) VALUES ('{faixa.id}', '{interprete_id}')"
            DataBaseService().insert(sql_query_faixa_interp)
    ...
    def show_faixas(self):
        sql_query = f"descricao FROM faixa"
